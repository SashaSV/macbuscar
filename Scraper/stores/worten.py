...
def build_search_url(product_name: str, cat: str) -> str:
    ...
    `cat` is currently unused — Worten doesn't have a department filter."""
    query = quote_plus(f'{product_name} Apple')
    return SEARCH_URL_TPL.format(host=HOST, query=query)


# English → Spanish color translation for fallback queries.
COLOR_TRANSLATIONS = {
    'blue':    'Azul',
    'silver':  'Plata',
    'green':   'Verde',
...
def parse_price(text: str):
    """Parse Spanish price string ('1.299,99 €') to float. None on failure."""
    if not text:
        return None
...
def is_captcha(html: str) -> tuple:
    ...
    Worten is fronted by DataDome — the script tag for DataDome may be present
...
def _slug_from_url(url: str) -> str:
    ...
    /productos/iphone-16-pro-256gb-titanio-natural-9876 → that whole tail."""
    if not url:
        return ''
...
# we need as `data-cnstrc-*` attributes — no fragile DOM scraping required.
#
...
def inspect_page(html: str) -> None:
    ...
    for r in parsed[:8]:
        ...
        print(f'     · [{r["asin"][:24]:24}] {r["name"][:60]:60} | {r["price"]}€{oldp}')
...
def subfamily_info(product: dict, variant: dict):
    """Same sub-family disambiguation as Amazon — product family + display
    size drives the search query and post-filter regex.

    NOTE: the regex matches a result NAME (a string in Spanish), so the same
    patterns work for Amazon and Worten — they list Apple products with the
...
def score_result(result: dict, variant: dict) -> int:
    ...
    + bandSize 30 + display 20 + connectivity ±15. Hard rejects on
...
def upsert_scraped_and_price(cur, variant_id: int, result: dict, cat: str, score: int):
    ...
    Amazon and Worten — the field name is shared for code reuse)."""
    sku      = result['asin']
    name     = result['name']
    url      = result['url']
        ...
        for i, product in enumerate(products, 1):
            ...
            for query, group in groups.items():
                ...
                if group_matched == 0 and results:
                    print(f'      🔍 No matches in this group. First 3 candidates returned by Worten:')
                    for r in results[:3]:
                        print(f'           · {r["name"][:120]}')
                    ...
                    for variant in unmatched_in_group:
                        ...
                        if best and best['asin'] in claimed_skus:
                            ...
                            print(f'            ⚠️  [{variant["id"]:4}] '
                                  f'{variant["nombre"][:60]} — fallback SKU '
